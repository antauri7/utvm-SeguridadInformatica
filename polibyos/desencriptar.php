    ...
    <tr>
      ...
      <?php
	  	if(isset($_POST["btnBuscar"]))
		{
			...
			$select = "select password from polibyosuser where usuario='".$recNombre."'";
			$querySelect = mysql_query($select);
			$valores = mysql_fetch_array ($querySelect);	
			
			$caracter=$valores['password'];
			$conver="";
			$con=strlen($caracter);
			for ( $i = 0 ; $i < $con ; $i ++) 
			{
				$imprime= substr($caracter,$i*2,2);
				
				$select = "select letra from poliregreso where cambio ='".$imprime."'";
				$query = mysql_query($select);
				
				while($renglon = mysql_fetch_array($query))
				{
					$conver = $conver.$renglon['letra'];
				}
			}
		}
